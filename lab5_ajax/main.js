...
function downloadData(page = 1, query = '') {
    ...
    xhr.onload = function () {
        ...
        renderSearchResults(this.response.records, query);
    };
    xhr.send();
}
...
function renderSearchResults(records, query) {
    ...
    let searchResults = records.filter(record => record.text.includes(query));

    if (searchResults.length > 0) {
        searchResults.forEach(result => {
            let resultElement = createListItemElement(result);
...
function autocomplete(query) {
    let autocompleteUrl = `http://cat-facts-api.std-900.ist.mospolytech.ru/autocomplete?q=${encodeURIComponent(query)}`;
    let autocompleteContainer = document.querySelector('.autocomplete');

    fetch(autocompleteUrl)
        .then(response => response.json())
        .then(data => {
            renderAutocomplete(data);
            autocompleteContainer.style.display = data.length > 0 ? 'block' : 'none';
        })
        .catch(error => {
